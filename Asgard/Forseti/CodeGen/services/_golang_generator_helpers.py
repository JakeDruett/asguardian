...
def json_type_to_go(schema: dict[str, Any], required: bool = True) -> str:
    ...
    if "$ref" in schema:
        type_name = sanitize_identifier(cast(str, schema["$ref"]).split("/")[-1])
...
def generate_go_type(type_def: TypeDefinition, include_documentation: bool, to_pascal_case_fn: Any) -> list[str]:
    ...
    if type_def.description and include_documentation:
        lines.append(f"// {type_name} {escape_line_comment(type_def.description)}")
    ...
    if type_def.is_enum:
        ...
        for value in type_def.enum_values:
            if isinstance(value, str):
                const_name = sanitize_identifier(
                    f"{type_name}{value.replace('-', '').replace('_', '').title()}"
                )
                lines.append(f"\t{const_name} {type_name} = {string_literal(value)}")
            else:
                const_name = sanitize_identifier(f"{type_name}{value}")
        ...
        for prop_name, prop_def in type_def.properties.items():
            go_name = sanitize_identifier(to_pascal_case_fn(prop_name))

            if prop_def.description and include_documentation:
                lines.append(f"\t// {escape_line_comment(prop_def.description)}")

            lines.append(f"\t{go_name} {prop_def.type_name} {go_json_tag(prop_name)}")
...
def generate_go_models_file(types: dict[str, TypeDefinition], spec_data: dict[str, Any], config: CodeGenConfig, to_pascal_case_fn: Any) -> GeneratedFile:
    ...
    lines.append(f"// {escape_line_comment(str(info.get('title', 'API')))} - Model Definitions")
    lines.append("//")
    lines.append("// Generated by Forseti CodeGen")
    if info.get("version"):
        lines.append(f"// API Version: {escape_line_comment(str(info.get('version')))}")
    lines.append("")
