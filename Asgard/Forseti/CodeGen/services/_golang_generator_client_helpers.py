...
def generate_go_api_method(method: MethodDefinition, config: CodeGenConfig, to_camel_case_fn: Any, to_pascal_case_fn: Any) -> list[str]:
    ...
    if config.include_documentation and method.description:
        lines.append(f"// {method_name} {escape_line_comment(method.description)}")
    ...
    lines.append(f"func (c *Client) {method_name}({params_str}) ({return_type}) {{")

    query_params = [p for p in method.parameters if p.location == "query"]
    lines.extend(go_path_lines(method.path, method.parameters, to_camel_case_fn))

    if query_params:
        lines.append("\tquery := url.Values{}")
        for param in query_params:
            go_name = sanitize_identifier(to_camel_case_fn(param.name))
            if "*" in param.type_name:
                lines.append(f"\tif {go_name} != nil {{")
                lines.append(f"\t\tquery.Set({string_literal(param.name)}, fmt.Sprintf(\"%v\", *{go_name}))")
                lines.append("\t}")
            else:
                lines.append(f"\tquery.Set({string_literal(param.name)}, fmt.Sprintf(\"%v\", {go_name}))")
    else:
    ...
    lines.append(f"\terr := c.doRequest({string_literal(method.http_method)}, path, query, {body_arg}, {result_arg})")
...
def generate_go_client_file(methods: list[MethodDefinition], types: dict[str, TypeDefinition], spec_data: dict[str, Any], config: CodeGenConfig, to_camel_case_fn: Any, to_pascal_case_fn: Any) -> GeneratedFile:
    ...
    lines.append(f"// {escape_line_comment(str(info.get('title', 'API')))} - API Client")
    lines.append("//")
