...
from Asgard.Forseti.CodeGen.services._python_generator_parse_helpers import (
    json_type_to_python,
    operation_to_method,
    parse_parameters,
)
...
class PythonGeneratorService:
    ...
    def generate(
        ...
        if self.config.generate_models:
            generated_files.append(generate_models_file(types, spec_data, self.config))

        if self.config.generate_client:
            generated_files.append(generate_client_file(methods, types, spec_data, self.config))

        generated_files.append(self._generate_init_file(generated_files))
    ...
    def _schema_to_property(
        ...
        type_name = json_type_to_python(schema)
        is_array = schema.get("type") == "array"
        array_item_type = None

        if is_array and "items" in schema:
            array_item_type = json_type_to_python(schema["items"])
    ...
    def _parse_methods(
        ...
        for path, path_item in paths.items():
            ...
            for method in http_methods:
                ...
                method_def = operation_to_method(
                    path, method, operation, path_params,
                    self._generate_method_name, self._to_snake_case,
                )
