...
class MockServerGeneratorService:
    ...
    def _generate_fastapi_server(self, server_def: MockServerDefinition) -> list[GeneratedFile]:
        ...
        routes_str = "\n\n".join(routes)
        server_code = f'''"""
{escape_docstring(server_def.title)} - Mock Server (FastAPI)
...
app = FastAPI(
    title={string_literal(server_def.title)},
    description={string_literal(server_def.description or "")},
    version={string_literal(str(server_def.version))},
)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host={string_literal(str(self.config.host))}, port={int(self.config.port)})
'''
    ...
    def _generate_express_server(self, server_def: MockServerDefinition) -> list[GeneratedFile]:
        ...
        routes_str = "\n\n".join(routes)
        server_code = f'''/**
 * {escape_block_comment(server_def.title)} - Mock Server (Express)
 * Generated by Forseti MockServer
 *
 * {escape_block_comment(server_def.description or "")}
 */
...
app.listen(PORT, {string_literal(str(self.config.host))}, () => {{
    console.log(`Mock server running at http://` + {string_literal(str(self.config.host))} + `:${{PORT}}`);
}});
'''
